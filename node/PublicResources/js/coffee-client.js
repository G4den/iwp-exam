...
function requestCoffeeStatus(event) {
  ...
  jsonFetch(
    document.getElementById("coffeeStatusForm_id").action +
      "/" +
      coffeeStatusFormData.orderID,
    coffeeStatusFormData
  )
    .then((coffeeStatus) => {
      console.log("Status=");
      console.log(coffeeStatus);
      let statusElem = document.getElementById("status_id");
      statusElem.textContent = `Hi ${
        coffeeStatus.name
      }! Your order was received at ${coffeeStatus.orderTime}. Your order is ${
        coffeeStatus.isReady ? "" : "not"
      } ready !`;
